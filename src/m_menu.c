...
void M_QuitDOOM(int choice)
{
  ...
  // Ty 03/27/98 - externalized DOSA as a string s_DOSA that's in the sprintf
  if (language != english)
    sprintf(endstring,"%s\n\n%s",s_DOSA, endmsg[0] );
  else         // killough 1/18/98: fix endgame message calculation:
    sprintf(endstring,"%s\n\n%s", endmsg[gametic%(NUM_QUITMESSAGES-1)+1], s_DOSA);
...
int M_GetKeyString(int c,int offset)
{
  ...
  menu_buffer[offset] = 0;
      ...
      case KEYD_PAGEUP:     s = "L2"; break;
      case KEYD_END:        s = "END";  break;
      case KEYD_PAGEDOWN:   s = "R2"; break;
...
*/
      ...
      else if (ch == key_menu_escape)                    // phares 3/7/98
  {
    ...
    strcpy(&savegamestrings[saveSlot][0],saveOldString);
      ...
      else if (ch == KEYD_LCTRL)                     // phares 3/7/98
  {
    saveStringEnter = 0;
...
//e6y
  ...
  if (setup_active) {
    ...
    if (default_verify)
      {
  if (toupper(ch) == KEYD_LCTRL) {
    M_ResetDefaults();
    default_verify = false;
    M_SelectDone(ptr1);
  }
  else if (toupper(ch) == KEYD_LALT) {
    default_verify = false;
    M_SelectDone(ptr1);
  }
  return true;
  ...
  if (ch == KEYD_LCTRL)                            // phares 3/7/98
    {
      if (currentMenu->menuitems[itemOn].routine &&
